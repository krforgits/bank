...
<?php 
  if(isset($_POST['form_submitted'])){
      ...
      if(empty($ACCID)  || empty($EMAIL)){
        echo "<script> alert('Empty Fields !!');
        window.location.href='ee.php';
        </script>";  
        exit() ;           
      }
      ...
      if(!ctype_digit($ACCID) || !ctype_graph($EMAIL)){
      ...
      $d = $r->fetch_row();
      if($d[0]<1){
        echo "<script> alert('Account no doesn't exists !!');
        window.location.href='ee.php';
        </script>";  
        exit() ;      
      }
          ...
          $InsertTable ="UPDATE `accountdetails` SET `email` = '$EMAIL' WHERE `accountdetails`.`accID` = $ACCID;";
