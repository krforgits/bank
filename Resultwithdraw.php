  ...
  if(isset($_POST['form_submitted'])){
      ...
      if($AMOUNT <=0){
       ...
       if($PAYER_ID == $PAYEE_ID ){
        echo "<script> alert('Can't withdraw from cash !!');
        window.location.href='Withdraw.php';
        </script>";  
        exit() ;  
      }
      if(!ctype_digit($AMOUNT) || !ctype_digit($PAYER_ID)){
      ...
      $d = $r->fetch_row();
      if($d[0]<1){
        echo "<script> alert('Withdrawer ID does not exists !!');
        window.location.href='Withdraw.php';
        </script>";  
        exit() ;      
      }
      ...
      $sql = "Select * from accountdetails where accID='$PAYER_ID'";       
          if($result = $conn->query($sql)){            
               ...
               if($row1['balance']<$AMOUNT){
                echo "<script> alert('Withdrawer does not have required balance !!');
                window.location.href='Withdraw.php';
                </script>";  
                exit() ; 
                }  
      ...
      $sql21 = "Select * from accountdetails where accID='$PAYEE_ID'";       
          if($result = $conn->query($sql21)){            
               $row1 = $result->fetch_array(); 
               if($row1['balance']<$AMOUNT){
                echo "<script> alert('Bank does not have required Cash !!');
                window.location.href='Withdraw.php';
                </script>";  
                exit() ; 
                }  
          ...
          echo "<h1 style='text-align: center'>Transaction Successfully Completed</h1>
                <p  style='text-align: center; font-size:25px;'>Details of Withdrawer and cash are as follows<p>
                <table id = 'Table'>
                <tr>
                <th></th>
                <th>Account No</th>
         ...
         $Uppayer ="Insert into PB$row1[sno] (type, des, amt, balance, time) values ('debit','Cash Withdrawal','$AMOUNT','$PayerCurrentBalance','$date')";
