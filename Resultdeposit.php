...
<?php 
  if(isset($_POST['form_submitted'])){
      ...
      if (empty($PAYER_ID) || empty($AMOUNT)){
      ...
      if ($PAYER_ID == $PAYEE_ID){
        echo "<script> alert('Depositer ID can't be cash!!');
        window.location.href='deposit.php';
        </script>";  
        exit() ;           
      }
      ...
      if(!ctype_digit($AMOUNT) ||  !ctype_digit($PAYEE_ID)){
      ...
      $d = $r->fetch_row();
      if($d[0]<1){
        echo "<script> alert('Depositer ID does not exists !!');
        window.location.href='deposit.php';
        </script>";  
        exit() ;      
      }
      ...
      $sql = "Select * from accountdetails where accID='$PAYER_ID'";       
          if($result = $conn->query($sql)){            
               ...
               if($row1['balance']<$AMOUNT){
                echo "<script> alert('Not enough cash!!');
                window.location.href='deposit.php';
                </script>";  
                exit() ; 
                }  
          ...
          echo "<h1 style='text-align: center'>Transaction Successfully Completed</h1>
                <p  style='text-align: center; font-size:25px;'>Details of cash and depositer are as follows<p>
                <table id = 'Table'>
                <tr>
                <th></th>
                <th>Account No</th>
          ...
          if($result = $conn->query($sql2)){
           ...
           if($conn->query($updatepayer)==true){
                ?>         
                <script>console.log("Cash UPDATED!!")</script>
                <?php
           }
           else{
                ?>        
                <script>alert("Cash NOT UPDATED!!")</script>
                <?php
           }

           if($conn->query($updatepayee)==true){
                    ?>         
                    <script>console.log("DEPOSITER DETAILS UPDATED! ")</script>
                    <?php
            }
            else{
                    ?>        
                    <script>alert("DEPOSITER DETAILS NOT UPDATED! ERROR OCCURED!")</script>
                    <?php
            }
            ...
            $InsertTransactTable ="Insert into history (payer, payerAcc, payee, payeeAcc, amount, time) values ('Cash','$row1[accID]','$row2[name]','$row2[accID]','$AMOUNT','$date')";
 ...
 $Uppayee ="Insert into PB$row2[sno] (type, des, amt, balance, time) values ('credit','Cash Deposit','$AMOUNT','$PayeeCurrentBalance','$date')";
